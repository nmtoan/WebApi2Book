    ...
    public class NamespaceHttpControllerSelector : IHttpControllerSelector
    {
        ...
        public HttpControllerDescriptor SelectController(HttpRequestMessage request)
        {
            ...
            var controllerKey = String.Format(CultureInfo.InvariantCulture, 
                "{0}.{1}", namespaceName, controllerName);
        ...
        private T GetRouteVariable<T>(IHttpRouteData subRouteData, string name)
        {
            object result;
            if (subRouteData.Values.TryGetValue(name, out result))
            {
                return (T)result;
            }
            return default(T);
